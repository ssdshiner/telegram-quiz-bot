...
PUBLIC_GROUP_COMMANDS = [
    'todayquiz', 'askdoubt', 'answer', 'section', 'alldoubts'
    'feedback'
]
...
minutes = int(seconds // 60)
        ...
        try:
            ...
            ""
            "
            Fetches a random, unused question and posts it as a quiz.This version now saves its state
            for correct answer validation.
            ""
            "
            if not supabase: return try:
                response = supabase.table('questions').select('*').eq('used', False).limit(50).execute() if not response.data:
            print("ℹ️ No unused questions for daily quiz. Resetting.") supabase.table('questions').update({
                'used': False
            }).neq('id', 0).execute() response = supabase.table('questions').select('*').eq('used', False).limit(50).execute() if not response.data:
            report_error_to_admin("Daily Quiz Failed: No questions found in the database.") return

            quiz_data = random.choice(response.data) question_id = quiz_data['id'] question_text = quiz_data.get('question_text', 'No question text provided.') options = quiz_data.get('options', []) correct_index = quiz_data.get('correct_index') explanation_text = quiz_data.get('explanation')
            ...
            # THE FIX: Save the correct answer to memory
            for the poll handler.QUIZ_SESSIONS[poll.poll.id] = {
            ...
            bot.send_message(
                ...
                'used': True
            }).eq('id', question_id).execute() print(f "✅ Daily quiz posted using question ID: {question_id}")

            except Exception as e:
            print(f "❌ Failed to post daily quiz: {e}") report_error_to_admin(f "Failed to post daily quiz:\n{traceback.format_exc()}")
    ...
    escape_chars = r'_*` ['#
                ...
                def check_membership(user_id):
                    ...
                    while True:
                    try: #Get the current time in IST once at the start of the loop
                    for consistency.
                    ...
                    if unanswered_count and unanswered_count > 0: #THE FIX: Removed unnecessary backslashes
                    ...
                    if isinstance(close_time, datetime.datetime): #Ensure the close_time is timezone - aware
                    ...
                    # -- -Process Scheduled Tasks(e.g., /notify follow-up) ---
                        for task in scheduled_tasks[: ]: #THE BUG FIX: Compare the task 's run_at time with the current IST time.#
                        This ensures scheduled tasks run correctly according to Indian time.if current_time_ist >= task['run_at'].astimezone(ist_tz):
                        try:
                        ...
                        finally: #THE IMPROVEMENT: Save data outside the main
                        try block.#This ensures data is saved even
                        if one of the tasks above had a minor, non - fatal error.save_data() time.sleep(30)# Wait
                        for 30 seconds before the next loop# === === === === === === === === === === === === === === === === === === === === === === === === === == #7. FLASK WEB SERVER & WEBHOOK# === === === === === === === === === === === === === === === === === === === === === === === === === ==
            ...
            escape_chars = r'_*` ['
                            ...
                            quotes = [# === === === === === === === === === === === === === === === == #-- -Hinglish & Relatable Quotes
                                for CA Students-- -# === === === === === === === === === === === === === === === ==
                                ...
                                "💹 **SFM:** Derivatives jitne complex lagte hain, utne hote nahi. Bas 'underlying asset' (concept) ko samajh lo.", # === === === === === === === === === === === === === === === == #-- -Quotes from Famous Personalities-- -# === === === === === === === === === === === === === === === ==
                            ...
                            # Send a random quote from the master list bot.send_message(GROUP_ID, random.choice(quotes), parse_mode = "Markdown") bot.send_message(msg.chat.id, "✅ Motivation sent to the group.")
                            ...
                            tips = [# === === === === === === === === === === === === === === === == #-- -Advanced Scientific Study Techniques-- -# === === === === === === === === === === === === === === === ==
                            ...
                            # Send a random tip from the master list tip = random.choice(tips) bot.send_message(GROUP_ID, tip, parse_mode = "Markdown") bot.send_message(msg.chat.id, "✅ Study tip sent to the group")
                            ...
                            "
                            keywords = [word
                                for word in question_text.split() if len(word) > 4
                            ]
                            if not keywords:
                            return None
                            query_string = " | ".join(keywords) try: #Find the single best match from doubts that have a 'best_answer_text'
                            ...
                            .limit(1)\
                            .execute() return response.data[0]
                            if response.data
                            else None
                            except Exception as e:
                            print(f "Note: Text search for related doubts failed: {e}") return None
                            ...
                            ""
                            try:
                            student_name = user.first_name student_id = user.id
                            ...
                            print(f "Error in create_new_doubt: {traceback.format_exc()}") bot.send_message(
                                chat_id,
                                "❌ Oops Something went wrong while creating your doubt. Please try again."
                            )


                            @ bot.message_handler(commands = ['askdoubt'])@ membership_required def handle_askdoubt(msg: types.Message):
                            ""
                            "Handles the /askdoubt command, now with an interactive confirmation flow."
                            ""
                            if not is_group_message(msg):
                            bot.reply_to(msg, "This command can only be used in the main group.") return
                            ...
                            priority = 'high'
                            question_text = command_text[6: ].strip() elif command_text.lower().startswith('[urgent]'):
                            priority = 'urgent'
                            ...
                            if related_doubt: #If a similar doubt is found, ask the user
                            for confirmation markup = types.InlineKeyboardMarkup()# Pass necessary info in callback_data yes_callback = f "show_ans_{related_doubt['id']}"
                            no_callback = f "ask_new_{hash(question_text)}"#
                            Use hash to keep it short markup.add(
                            ...
                            # Store the user 's question temporarily for the '
                            No ' option
                            ...
                            #
                            Escape all variable content safe_user_name = escape_markdown(msg.from_user.first_name) safe_question_preview = escape_markdown(
                            ...
                            s message yet, we wait
                            for their choice.else: #If no related doubt is found, create a new one directly create_new_doubt(msg.chat.id, msg.from_user, question_text, priority) bot.delete_message(msg.chat.id, msg.message_id)


                            @ bot.callback_query_handler(func = lambda call: call.data.startswith('show_ans_') or call.data.startswith('ask_new_')) def handle_doubt_confirmation(call: types.CallbackQuery):
                            ""
                            ...
                            for the related doubt response = supabase.table('doubts').select('best_answer_text').eq(
                                'id', doubt_id).limit(1).execute() if response.data and response.data[0]['best_answer_text']:
                            best_answer = response.data[0]['best_answer_text'] bot.edit_message_text(
                            ...
                            original_doubt_data = user_states.get(f "doubt_{question_hash}") if original_doubt_data:
                            bot.edit_message_text("Okay, posting it as a new doubt for you",
                                call.message.chat.id,
                                call.message.message_id) create_new_doubt(call.message.chat.id, call.from_user,
                                original_doubt_data['question'],
                                original_doubt_data['priority'])# Clean up the state del user_states[f "doubt_{question_hash}"]
                            else :
                                bot.edit_message_text(
                            ...
                            @bot.message_handler(commands = ['answer'])@ membership_required def handle_answer(msg: types.Message):
                            ""
                            "Handles the /answer command. Uses send_message for error feedback."
                            ""
                            if not is_group_message(msg):
                            bot.send_message(msg.chat.id,
                                "This command can only be used in the main group.") return

                            try:
                            parts = msg.text.split(' ', 2) if len(parts) < 3:
                            bot.send_message(
                                msg.chat.id,
                                "Invalid format. Use: `/answer [Doubt_ID] [Your Answer]`\n*Example:* `/answer 101 The answer is...`",
                                parse_mode = "Markdown") return

                            doubt_id = int(parts[1]) answer_text = parts[2].strip()

                            fetch_response = supabase.table(
                                'doubts').select('message_id, all_answer_message_ids').eq(
                                'id', doubt_id).limit(1).execute() if not fetch_response.data:
                            bot.send_message(msg.chat.id,
                                f "❌ Doubt with ID #{doubt_id} not found.") return

                            doubt_data = fetch_response.data[0] original_message_id = doubt_data['message_id'] all_answers = doubt_data.get('all_answer_message_ids', [])

                            # This is the critical part: escaping the user 's name and their answer text.
                            safe_answerer_name = escape_markdown(msg.from_user.first_name) safe_answer_text = escape_markdown(answer_text)

                            # Sending the reply to the original doubt message sent_answer_msg = bot.send_message(
                                chat_id = msg.chat.id,
                                text = f "↪️ *Answer by {safe_answerer_name}:*\n\n{safe_answer_text}",
                                reply_to_message_id = original_message_id,
                                parse_mode = "Markdown")

                            all_answers.append(sent_answer_msg.message_id) supabase.table('doubts').update({
                            ...
                            bot.send_message(msg.chat.id,
                                ...
                                reply_to_message_id = msg.message_id) except Exception as e:
                            print(f "Error in /answer: {traceback.format_exc()}") bot.send_message(
                                msg.chat.id,
                                "❌ Oops Something went wrong while submitting your answer.",
                                reply_to_message_id = msg.message_id)

                            @ bot.message_handler(commands = ['bestanswer'])@ membership_required def handle_best_answer(msg: types.Message):
                            ""
                            ...
                            try:
                            parts = msg.text.split(' ', 1) if len(parts) < 2 or not msg.reply_to_message:
                            bot.reply_to(
                                msg,
                                "Invalid format. Use: `/bestanswer [Doubt_ID]` by *replying* to the best answer message.",
                                parse_mode = "Markdown") return

                            doubt_id = int(parts[1]) best_answer_msg = msg.reply_to_message

                            #...(rest of the
                                function is the same)...fetch_response = supabase.table('doubts').select('*').eq('id', doubt_id).limit(1).execute() if not fetch_response.data:
                            bot.reply_to(msg, f "❌ Doubt with ID #{doubt_id} not found.") return

                            doubt_data = fetch_response.data[0]

                            if not(msg.from_user.id == doubt_data['student_id'] or is_admin(msg.from_user.id)):
                            bot.reply_to(msg, "❌ You can only mark the best answer for your own doubt.") return

                            best_answer_text = best_answer_msg.text.split(':', 1)[-1].strip() updated_message_text = (
                            ...
                            try:
                            bot.unpin_chat_message(msg.chat.id, doubt_data['message_id']) except Exception as e:
                            ...
                            all_answer_ids = doubt_data.get('all_answer_message_ids', []) for answer_id in all_answer_ids:
                            try:
                            bot.delete_message(msg.chat.id, answer_id) except Exception:
                            pass

                            bot.delete_message(msg.chat.id, msg.message_id)

                            except Exception as e:
                            print(f "Error in /bestanswer: {traceback.format_exc()}") bot.reply_to(msg, "❌ Oops! Something went wrong.")@ bot.message_handler(commands = ['alldoubts'])@ membership_required def handle_all_doubts(msg: types.Message):
                            ""
                            "
                            Fetches and lists all unanswered doubts
                            for any group member.Includes instructions on how to reply and a tip
                            for the original poster.
                            ""
                            "
                            try: #Fetch up to 15 unanswered doubts, newest first.response = supabase.table('doubts').select('id, question').eq('status', 'unanswered').order('id', desc = True).limit(15).execute() if not response.data:
                            bot.send_message(
                                msg.chat.id,
                            ...
                            doubt_id = doubt.get('id')# Truncate the question to keep the list clean question_preview = doubt.get('question', '')[: 70] safe_question_preview = escape_markdown(question_preview)

                            message_text += f "*#Doubt{doubt_id}:* _{safe_question_preview}..._\n"#
                            NEW: Clearer, pre - formatted instruction on how to reply message_text += f "_Reply with:_ `/answer {doubt_id} [Your Answer]`\n\n"

                            #
                            Add a horizontal line
                            for separation message_text += "--- \n"#
                            NEW: Add the tip at the end of the message message_text += (
                            ...
                            # Send the complete list to the user who asked bot.send_message(msg.chat.id, message_text, parse_mode = "Markdown")

                            except Exception as e:
                            print(f "Error in /alldoubts command: {traceback.format_exc()}") report_error_to_admin(f "Failed to fetch doubt list:\n{traceback.format_exc()}") bot.send_message(msg.chat.id, "😥 Oops! Something went wrong while fetching the doubt list.")# === === === === === === === === === === === === === === === === === === === === === === === === === == #17 ADVANCED QUIZ MARATHON FEATURE(FULLY CORRECTED AND ROBUST)# === === === === === === === === === === === === === === === === === === === === === === === === === ==
                            ...
                            # THE FIX: Determine the correct number of questions that were actually asked.#This is the current question index, which is accurate even
                            if the marathon was stopped.total_questions_asked = session.get('current_question_index', 0)

                            # Mark only the questions that were actually used as 'used'.if session.get('questions') and total_questions_asked > 0:
                            try: #Get the IDs of questions from index 0 up to the last one asked.used_question_ids = [q['id']
                            ...
                            name = escape_markdown(p['name'])# THE FIX: Calculate percentage based on questions asked, not the total planned.percentage = (p['score'] / total_questions_asked * 100) if total_questions_asked > 0
                            else 0
                            ...
                            bot.send_message(GROUP_ID, insights_text, parse_mode = "Markdown")# === === === === === === === === === === === === === === === === === === === === === === === === === == #8. Y.UNIFIED POLL ANSWER HANDLER(SIMPLIFIED & FINAL VERSION)# === === === === === === === === === === === === === === === === === === === === === === === === === ==

                            @bot.poll_answer_handler() def handle_all_poll_answers(poll_answer: types.PollAnswer):
                            ""
                            "
                            ...
                            for marathon answers

                            #-- -ROUTE 2: Random Quiz / Daily Quiz-- -#We now correctly check the 'active_polls'
                            list
                            for these quizzes.else:
                            active_poll_info = next((poll
                                for poll in active_polls
                                if poll['poll_id'] == poll_id_str), None)

                            # If we found the poll and it 's a random quiz type...
                            if active_poll_info and active_poll_info.get('type') == 'random_quiz': #Check
                            if the user 's answer is the correct one
                            if selected_option == active_poll_info['correct_option_id']:
                            print(f "Correct answer for random/daily quiz from {user_info.first_name}. Incrementing score.")# If it 's correct, call our Supabase function
                            supabase.rpc('increment_score', {
                                'user_id_in': user_info.id,
                                'user_name_in': user_info.first_name
                            }).execute()# No
                            return here, allowing
                            for other potential poll types in the future
                            if needed

                            except Exception as e:
                            print(f "Error in the master poll answer handler: {traceback.format_exc()}") report_error_to_admin(f "Error in handle_all_poll_answers:\n{traceback.format_exc()}")@ bot.message_handler(content_types = ['new_chat_members']) def handle_new_member(msg: types.Message):
                            ""
                            "
